(function(){
    ...
    `;
    // c = document.querySelector("#particle");
    c.width = 1920;
    c.height = 1080;
    x = c.getContext("2d");
    C = Math.cos;
    ...
    rsz = window.onresize = () => {
        setTimeout(() => {
            if (document.body.clientWidth > document.body.clientHeight * 1.77777778) {
                ...
                c.style.width = "100vw";
                setTimeout(
                    () => (c.style.width = c.clientHeight * 1.77777778 + "px"),
                    0
                );
            } else {
                c.style.width = "100vw";
                setTimeout(
                    () => (c.style.height = c.clientWidth / 1.77777778 + "px"),
                    0
                );
            }
        }, 0);
    ...
    async function Draw() {
        ...
        if (!t) {
                    ...
                    B.map((v, i) => {
                        ...
                        B.map((q, j) => {
                            if (j != i) {
                                ...
                                d =
                                    1 +
                                    (Math.hypot(X - X2, Y - Y2, Z - Z2) * (3 + iBc / 40) * 3) **
                                    4;
                                X += ((X - X2) * 500) / d;
                                Y += ((Y - Y2) * 500) / d;
                                Z += ((Z - Z2) * 500) / d;
            ...
            lineFaceI = (
                ...
                let p2_ = -(
                    Math.acos(
                        (Y2_ - Y1_) / (Math.hypot(X2_ - X1_, Y2_ - Y1_, Z2_ - Z1_) + 0.001)
                    ) +
                    Math.PI / 2
                );
                let isc = false,
            ...
            LsystemRecurse = (
                ...
                let p2a =
                    -Math.acos(
                        (Y2 - Y1) / (Math.hypot(X2 - X1, Y2 - Y1, Z2 - Z1) + 0.0001)
                    ) + Math.PI;
            ...
            spawnP = () => {
                base_shp.map((v, i) => {
                    ...
                    (ax /= v.length), (ay /= v.length), (az /= v.length);
                    let b1 = v[2][0] - v[1][0],
                        b2 = v[2][1] - v[1][1],
                        b3 = v[2][2] - v[1][2];
                    let c1 = v[1][0] - v[0][0],
                    ...
                    let X1_ = ax,
                        ...
                        Z1_ = az;
                    let flip = 1;
                    let d1_ = Math.hypot(X1_, Y1_, Z1_);
                    let d2_ = Math.hypot(
                        ax + crs[0] / 99,
                        ay + crs[1] / 99,
                        az + crs[2] / 99
                    ...
                    p1 = Math.atan2(X2_ - X1_, Z2_ - Z1_);
                    p2 =
                        Math.PI / 2 -
                        Math.acos(
                            (Y2_ - Y1_) /
                            (Math.hypot(X2_ - X1_, Y2_ - Y1_, Z2_ - Z1_) + 0.00000001)
                        );
                    for (m = iPc; m--;) {
                        X1 = 0;
                        Y1 = 0;
                        Z1 = 0;
                        X2 = S((p_ = Rn() * Math.PI * 2)) * 1e3;
                        Y2 = C(p_) * 1e3;
                        Z2 = 0;
                        f = false;
                        v.map((q, j) => {
                        ...
                        if (f) {
                            ...
                            vx =
                                ...
                                S(
                                    (q_ = mode
                                        ? Math.PI - Rn() ** 0.5 * Math.PI
                                        : Rn() ** 0.5 * Math.PI)
                                ) *
                                iPv;
                            vy = C(q_) * iPv;
                            vz = C(p) * S(q_) * iPv;
                            P = [...P, [px + tx_, py + ty_, pz + tz_, vx, vy, vz, 1, i]];
                        }
                    }
                });
            };
            rl = pt = yw = rlv = ptv = ywv = 0;
            shprv = 0.075;
            switch ((Rn() * 3) | 0) {
                case 0:
                    rlv = (Rn() - 0.5) * shprv;
        ...
        (oX = 0),
            (oY = 0),
            (oZ = Math.min(20, Math.max(5, (0.3 + C(t / 4)) * 100)));
        (Rl = C(t / 2)), (Pt = C(t / 2) / 2), (Yw = C(t / 4) * 4);
        ...
        base_shp.map((v, i) => {
            ...
            col2 = `hsla(${(360 / base_shp.length) * i * 3 + t * 50},99%,50%,.05)`;
            col1 = `hsla(${(360 / base_shp.length) * i * 3 + t * 50},99%,85%,.3)`;
            stroke(col1, col2, 2, true);
        });

        P = P.filter((v) => v[6] > 0.1);
        P.map((v) => {
            X = v[0] += v[3];
            Y = v[1] += v[4] += grav;
            Z = v[2] += v[5];
            ...
            if (Z > 0) {
                l = Q();
                s = Math.min(3e3, (500 / Z) * v[6]);
                x.fillStyle = `hsla(${-25 + (360 / base_shp.length) * v[7] * 3 + t * 50
                    },99%,50%,.025)`;
                x.fillRect(l[0] - s / 2, l[1] - s / 2, s, s);
                s /= 2;
                x.fillStyle = `hsla(${25 + (360 / base_shp.length) * v[7] * 3 + t * 50
